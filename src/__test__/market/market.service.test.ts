...
describe("MarketService", () => {
    ...
    it("should correctly calculate total_deposit and active_loans in USD", async () => {
        ...
        priceServiceMock.getPrice.mockImplementation(
    ...
    it("should return market detail with correct structure and filtered upcoming markets", async () => {
        ...
        tokenRepositoryMock.findByAssetId = jest
            .fn()
            .mockResolvedValue(mockAsset);
        priceServiceMock.getPrice.mockResolvedValue(50000);
        orderRepositoryMock.getBestRates.mockResolvedValue(new Map());
        marketRepositoryMock.getSumDepositByAssetId = jest
            .fn()
            .mockResolvedValue("100000000"); // 1 BTC
        marketRepositoryMock.getSumLoansByAssetId = jest
            .fn()
            .mockResolvedValue("50000000"); // 0.5 BTC

        const futureDate = new Date();
        futureDate.setFullYear(futureDate.getFullYear() + 1);

        const mockUpcomingMarkets = [{ id: "m1", maturity: futureDate }];
        marketRepositoryMock.getUpcomingMarkets = jest
            .fn()
            .mockResolvedValue(mockUpcomingMarkets);
        ...
        expect(marketRepositoryMock.getUpcomingMarkets).toHaveBeenCalledWith(
            assetId,
            3,
        );
    });
    ...
    it("should skip maturities within 7 days (inclusive) for getMarketSnapshot", async () => {
        ...
        marketRepositoryMock.getTotalDepositUsd.mockResolvedValue([]);
        marketRepositoryMock.getActiveLoans.mockResolvedValue([]);
        marketRepositoryMock.getEarliestMarketByAssetIds.mockResolvedValue([]);
