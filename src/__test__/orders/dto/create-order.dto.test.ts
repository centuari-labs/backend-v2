...
describe("CreateMarketOrderDto", () => {
    ...
    const createDto = (
        ...
        return plainToInstance(CreateMarketOrderDto, {
            ...validDto,
            ...overrides,
        });
    };
    ...
    describe("market order has no rate field", () => {
        ...
        it("should pass validation even if extra rate field is provided (not validated)", async () => {
            const dto = plainToInstance(CreateMarketOrderDto, {
                ...validDto,
                rate: 500,
            });
            const errors = await validate(dto);
    ...
    describe("amount validation", () => {
        ...
        it("should accept amount exactly at minimum (10 USD)", async () => {
            const dto = createDto({ amount: "10" });
            const errors = await validate(dto);
            const amountErrors = errors.filter((e) => e.property === "amount");
            expect(amountErrors).toHaveLength(0);
        });
...
describe("CreateLimitOrderDto", () => {
    ...
    const createDto = (
        ...
        return plainToInstance(CreateLimitOrderDto, {
            ...validDto,
            ...overrides,
        });
    };
