...
describe("OrdersService", () => {
    ...
    describe("cancelOrder", () => {
        ...
        it("should throw ForbiddenException when account not found for wallet", async () => {
            ...
            await expect(
                service.cancelOrder("uuid-cancel-006", mockWalletAddress),
            ).rejects.toThrow(ForbiddenException);
    ...
    describe("NATS publish failure handling", () => {
        const lendLimitDto: CreateLendLimitOrderDto = {
            loanToken: mockTokenAddress,
            amount: "1000",
            maturities: [1704067200],
        ...
        it("should still return saved order when NATS publish fails on create", async () => {
            const expectedOrder = createMockOrder();
            ...
            orderRepository.create.mockReturnValue(expectedOrder);
            orderRepository.save.mockResolvedValue(expectedOrder);
            natsService.publish.mockRejectedValue(new Error("NATS timeout"));
        ...
        it("should still return cancelled order when NATS publish fails on cancel", async () => {
            const openOrder = createMockOrder({
                id: "uuid-nats-fail",
                status: OrderStatus.Open,
            });
