...
export class RepayService {
    ...
    async repay(dto: RepayRequestDto, walletAddress: string, privyUserId: string): Promise<RepayResponseDto> {
        ...
        const accountId = await this.orderRepository.getOrCreateAccount(walletAddress, privyUserId).then(a => a.id);
        
        const market = await this.repayRepository.getMarketWithAsset(marketId);
        if (!market) throw new NotFoundException("Market not found");

        const totalDebtStr = await this.repayRepository.getUserTotalDebt(accountId, marketId);
        const totalDebtBaseUnits = BigInt(totalDebtStr);
        const repayAmountBaseUnits = this.parseRepayAmount(amount, market.decimals ?? 18, totalDebtBaseUnits);

        const positions = await this.repayRepository.getBorrowPositions(accountId, marketId);
        ...
        const maturityUnix = maturityDate ? Math.floor(maturityDate.getTime() / 1000) : 0;

        this.logger.log(`Executing repay: token=${market.tokenAddress}, user=${walletAddress}, amount=${repayAmountBaseUnits}, maturity=${maturityUnix}`);

        const txHash = await this.executeBlockchainRepay(walletAddress, market.tokenAddress, BigInt(maturityUnix), repayAmountBaseUnits);

        await this.updateDatabaseState(positions, repayAmountBaseUnits, txHash, accountId, marketId);
    ...
    private async updateDatabaseState(positions: any[], repayAmount: bigint, txHash: string, accountId: string, marketId: string): Promise<void> {
        try {
            await this.dataSource.transaction(async (manager) => {
                const targetPositions = await this.repayRepository.getBorrowPositions(accountId, marketId, manager);
                ...
                for (const pos of targetPositions) {
                    ...
                    await this.repayRepository.updateBorrowPositionDebt(manager, pos.id, newDebt.toString());
                    remaining -= toRepay;
                }
            });
